$(document).ready(function () {
    ...
    $("#Update").click(function (e) {
        ...
        $.each($('#left-defaults').find('li'), function () {
            if (industry == "") {
                industry = industry + $(this).attr("data-id");
            }
            else {
                industry = industry + "," + $(this).attr("data-id");
            }
        });
        Arrengeindustry(industry);
    });
});
function Arrengeindustry(industry) {
    var parameter = { "industry": industry };
    $.ajax({
        type: 'POST',
        url: "rearrange-industry.aspx/IndustryOrderUpdate",
...
function Populateindustry() {
    $.ajax({
        type: 'POST',
        url: "rearrange-industry.aspx/IndustryOrder",
        data: "{}",
        contentType: 'application/json; charset=utf-8',
        dataType: "json",
        async: false,
        success: function (data) {
            if (data.d != null) {
                ...
                for (var i = 0; i < data.d.length; i++) {
                    str = str + "<li data-id='" + data.d[i].Id + "' class='ui-state-default media  d-md-flex d-block text-sm-left text-center col-md-2'><div class='maindiv'><img src='/" + data.d[i].BannerImage + "' alt='" + data.d[i].IndustryName + "' class='img-responsive' style='max-width:100%;' /><div><span>" + data.d[i].IndustryName + "</span></div></div></li>";
                }
