...
public partial class Admin_page_group : System.Web.UI.Page
{
    ...
    public void GetGroups()
    {
        try
        {
            List<PageGroup> comps = PageGroup.GetAllPageGroup(conSQ).Where(x => x.Id == Convert.ToInt32(Request.QueryString["id"])).ToList();
            if (comps.Count > 0)
            {
                btnSave.Text = "Update";
        ...
        try
        {
            if (Page.IsValid)
            {
                ...
                if (btnSave.Text == "Update")
                {
                    ...
                    ca.Status = "Active";
                    int result = PageGroup.UpdatePageGroup(conSQ, ca);
                    if (result > 0)
                    {
                        GetAllGroup();
                        GetGroups();
                ...
                else
                {
                    ...
                    ca.UpdatedOn = TimeStamps.UTCTime();
                    ca.Status = "Active";
                    int result = PageGroup.InsertPageGroup(conSQ, ca);
                    if (result > 0)
                    {
    ...
    public void GetAllGroup()
    {
        ...
        {
            strPageGroups = "";
            List<PageGroup> cas = PageGroup.GetAllPageGroup(conSQ).OrderByDescending(s => Convert.ToDateTime(s.UpdatedOn)).ToList();
            int i = 0;
    ...
    public static string Delete(string id)
    {
        ...
        try
        {
            ...
            if (CreateUser.CheckAccess(conSQ, "page-group.aspx", "Delete", HttpContext.Current.Request.Cookies["pcc_aid"].Value))
            {
                ...
                int exec = PageGroup.DeletePageGroup(conSQ, cat);
                if (exec > 0)
                {
                    x = "Success";
                }
