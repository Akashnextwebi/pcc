...
public partial class Admin_view_seminar : System.Web.UI.Page
{
    public string strSeminars = "";
    ...
    public void GetAllDetails()
    {
        ...
        {
            strSeminars = "";
            List<SeminarDetails> BD = SeminarDetails.GetSeminarDetails(conSQ).ToList();
            for (int i = 0; i < BD.Count; i++)
            {
                ...
                strSeminars += @"<tr>
                                        <td>" + (i + 1) + @"</td>
                                        <td>" + image + @"</td>
                                        <td>" + BD[i].SeminarTitle + @"</td>
                                        <td>" + BD[i].Tag + @"</td>
                                         <td>" + BD[i].AddedOn.ToString("dd/MMM/yyyy") + @"</td>
                                        <td class='text-center'> 
                                            <a href='add-seminar.aspx?id=" + BD[i].Id + @"' class='bs-tooltip text-info fs-18' data-id='" + BD[i].Id + @"' data-toggle='tooltip' data-placement='top' title='Edit' data-original-title='Edit'>
    ...
    public static string Delete(string id)
    {
        ...
        try
        {
            ...
            int exec = SeminarDetails.DeleteSeminar(conSQ, BD);
            if (exec > 0)
            {
                x = "Success";
            }
