...
public partial class Admin_create_role : System.Web.UI.Page
{
            ...
            if (Page.IsValid)
            {
                ...
                if (btnSave.Text == "Update")
                {
                    if (CreateUser.CheckAccess(conSQ, pageName, "Edit", Request.Cookies["bmw_aid"].Value))
                    {
                        ...
                        ca.Status = "Active";
                        int result = UserRoles.UpdateUserRoles(conSQ, ca);
                        if (result > 0)
                        {
                            GetAllRoles();
                            GetRoles();
                ...
                else
                {
                    if (CreateUser.CheckAccess(conSQ, pageName, "Add", Request.Cookies["bmw_aid"].Value))
                    {
                        ...
                        ca.UpdatedOn = TimeStamps.UTCTime();
                        ca.Status = "Active";
                        int result = UserRoles.InsertUserRoles(conSQ, ca);
                        if (result > 0)
                        {
                            txtName.Text = "";
                            GetAllRoles();
    ...
    public void GetAllRoles()
    {
        ...
        {
            strUserRoless = "";
            List<UserRoles> cas = UserRoles.GetAllUserRoles(conSQ).OrderByDescending(s => Convert.ToDateTime(s.UpdatedOn)).ToList();
            int i = 0;
    ...
    public static string Delete(string id)
    {
        ...
        try
        {
            ...
            if (CreateUser.CheckAccess(conSQ, "create-role.aspx", "Delete", HttpContext.Current.Request.Cookies["bmw_aid"].Value))
            {
                ...
                int exec = UserRoles.DeleteUserRoles(conSQ, cat);
                if (exec > 0)
                {
                    x = "Success";
                }
