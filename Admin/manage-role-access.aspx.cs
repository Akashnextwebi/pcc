...
public partial class Admin_manage_role_access : System.Web.UI.Page
{
    ...
    public void GetRoleDetails()
    {
        try
        {
            List<UserRoles> comps = UserRoles.GetAllUserRoles(conSQ).Where(x => x.Id == Convert.ToInt32(Request.QueryString["id"])).ToList();
            if (comps.Count > 0)
            {
    ...
    public void GetUserAccess()
    {
        try
        {
            List<UserRolesAccess> users = UserRoles.GetAllUserRoles(conSQ, Request.QueryString["id"]).OrderBy(x => x.GOrderBy).ToList();
            int i = 0;
...
<div class='form-check form-switch changeAccess' id='v_" + rl.PageId + @"'  data-id='" + rl.PageId + @"'>
    ...
    public static string ChangeStatus(string pid, string rid, string v, string a, string e, string d)
    {
        ...
        try
        {
            ...
            SqlConnection conSQ = new SqlConnection(ConfigurationManager.ConnectionStrings["conSQ"].ConnectionString);
            List<UserRolesAccess> users = UserRoles.GetAllUserRolesCheck(conSQ, rid, pid);
            ...
            if (users.Count > 0)
            {
                if (CreateUser.CheckAccess(conSQ, "manage-role-access.aspx", "Edit", HttpContext.Current.Request.Cookies["pcc_aid"].Value))
                {
                    ...
                    int exec = UserRoles.UpdateRoleAccess(conSQ, ua);
                    if (exec > 0)
                    {
                        x = "Success";
                    }
            ...
            else
            {
                if (CreateUser.CheckAccess(conSQ, "manage-role-access.aspx", "Add", HttpContext.Current.Request.Cookies["pcc_aid"].Value))
                {
                    ...
                    ua.Status = "Active";
                    ua.PageGroupId = "";
                    int exec = UserRoles.InsertToRoleAccess(conSQ, ua);
                    if (exec > 0)
                    {
                        x = "Success";
                    }
